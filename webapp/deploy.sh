...
gcloud run deploy iso-builder \
    ...
    --max-instances 10 \
    --min-instances 0 \
    --set-env-vars="NODE_ENV=production,GCP_PROJECT_ID=$PROJECT_ID,GCP_ZONE=$ZONE,GCP_REGION=$REGION,GCS_ARTIFACTS_BUCKET=$ARTIFACTS_BUCKET,GCS_DOWNLOADS_BUCKET=$DOWNLOADS_BUCKET,VM_MACHINE_TYPE=c2d-highcpu-32,VM_BOOT_DISK_SIZE=500,VM_LOCAL_SSD_COUNT=4,MAX_CONCURRENT_BUILDS=3,BUILD_TIMEOUT_HOURS=6,STALLED_PROGRESS_MINUTES=30,VM_AUTO_CLEANUP=true,API_SECRET_KEY=$API_SECRET,LOG_LEVEL=info" \
